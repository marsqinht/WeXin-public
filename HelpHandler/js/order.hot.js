$(function () {
    ...
    $(window).scroll(function () {
        ...
        if ($(this).scrollTop() + $(window).height() + 100 >= $(document).height() && $(this).scrollTop() > 100) {
            ...
            if (stop == true) {
                ...
                $.getJSON('/api/MobileApi/HotPackageList?userId=' + userId + '&pageIndex=' + p + '&pageSize=10', function (data) {
                    if (data && data.IsSuccess && data.Data && data.Data.length > 0) {
                        var listHtml = ''
                        for (var dataL = 0; dataL < data.Data.length; dataL++) {
                            if (data.Data[dataL].WPackageStatus == '已完成' || data.Data[dataL].WPackageStatus == '已取消' || data.Data[dataL].WPackageStatus == '已过期') {
                                console.log(11);
                                continue;
                            }
    ...
    $.getJSON('/api/MobileApi/HotPackageList?userId=' + userId + '&pageIndex=' + 1 + '&pageSize=10', function (data) {
        if (data && data.IsSuccess && data.Data && data.Data.length > 0) {
            var listHtml = ''
            for (var dataL = 0; dataL < data.Data.length; dataL++) {
                if (data.Data[dataL].WPackageStatus == '已完成' || data.Data[dataL].WPackageStatus == '已取消' || data.Data[dataL].WPackageStatus == '已过期') {
