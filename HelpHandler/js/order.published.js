...
$(function () {
    ...
    $(window).scroll(function () {
        ...
        if ($(this).scrollTop() + $(window).height() + 100 >= $(document).height() && $(this).scrollTop() > 100) {
            ...
            if (stop == true) {
                ...
                $.getJSON('/api/MobileApi/PubPackageList?userId=' + userId + '&pageIndex=' + p + '&pageSize=10', function (data) {
                    if (data && data.IsSuccess && data.Data && data.Data.length > 0) {
                        ...
                        for (var dataL = 0; dataL < data.Data.length; dataL++) {
                            ...
                            + '         <div class="company-name"> ' + data.Data[dataL].PakcageTenant + ' <div class="order-prcie "><span>' + data.Data[dataL].PackagePrice + '</span>元/吨</div>   </div>               '
    ...
    $.getJSON('/api/MobileApi/PubPackageList?userId=' + userId + '&pageIndex=' + 1 + '&pageSize=10', function (data) {
        if (data && data.IsSuccess && data.Data && data.Data.length > 0) {
            var listHtml = ''
            for (var dataL = 0; dataL < data.Data.length; dataL++) {
                listHtml = listHtml
